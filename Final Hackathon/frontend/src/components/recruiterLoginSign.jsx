...
function RecruiterAuth() {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isLogin
      ? 'http://localhost:3000/api/recruiter/login'
      : 'http://localhost:3000/api/recruiter/signup';
    ...
    try {
      ...
      if (response.ok) {
        if (data.recruiterId) {
          sessionStorage.setItem('recruiterId', data.recruiterId);
        }
        setMessage({ type: 'success', text: isLogin ? 'Login successful!' : 'Signup successful!' });
        setFormData({ firstName: '', lastName: '', email: '', password: '', company: '' });
